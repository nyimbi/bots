...
class PictureOCR(Picture):
    ...
    def ocr_image(self):
        self.txt = tocr.image_to_text(self.img) or "Couldn't get a reading"
        
        
    def send(self):
        
        self.message.reply_text(self.txt)
...
def on_photo_private(bot, update):
    logger.info("OCR private chat photo")
    
    PictureOCR(update.message).one_shot()
...
def on_command(bot, update, args):
    ...
    PictureOCR(update.message, quality=quality_level).one_shot()
...
class module:
    ...
    handlers = (
        ...
        CommandHandler(["ocr", "extract", "text"],
